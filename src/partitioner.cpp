...
namespace yagkp::Partitioner {
	std::vector<int_t> GetGraphKPartition(const Graph& graph, const int_t k) {
		...
		for (int_t i = 0; i < ProgramConfig::runs; ++i) {
			std::vector<int_t> partition = RecursivePartition(graph, k, M, 0);
	...
	std::vector<int_t> RecursivePartition(const Graph& graph, const int_t k, const int_t M, int_t offset) {
		...
		std::vector<CoarseLevel> coarse_levels = Coarser::GetCoarseLevels(graph, k);
		...
		std::vector<int_t> partition1 = RecursivePartition(first_graph, k1, M, offset);
		std::vector<int_t> partition2 = RecursivePartition(second_graph, k2, M, offset + k1);
